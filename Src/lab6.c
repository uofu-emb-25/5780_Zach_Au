...
int lab6_main(void) {
    ...
    GPIOA->MODER |= (3 << (2 * 4));  // Set PA4 to analog mode
    GPIOA->PUPDR &= ~(3 << (2 * 4)); // No pull-up/pull-down

    // Set up DAC channel
    DAC->CR &= ~DAC_CR_EN1;           // Disable DAC channel 1
    DAC->CR &= ~DAC_CR_TSEL1;         // Select software trigger (000)
    ...
    while(1) {
        ...
        DAC->DHR8R1 = sine_table[i];   // Write 8-bit value to DAC channel 1
        i = (i + 1) % 32;             // Loop through 32-sample waveform
